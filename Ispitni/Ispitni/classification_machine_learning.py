...
if __name__ == '__main__':
    ...
    two = [row for row in dataset if row[-1] == 2]
    
    # Test_Set
    test_set = zero[int(len(zero) * x2):] + one[int(len(one) * x2):] + two[int(len(two) * x2):]
    test_x = [row[:-1] for row in test_set]
    test_y = [row[-1] for row in test_set]
    
    # Naive Bayes
    train_nb = zero[:int(len(zero) * x1)] + one[:int(len(one) * x1)] + two[:int(len(two) * x1)]
    train_nb_x = [row[:-1] for row in train_nb]
    train_nb_y = [row[-1] for row in train_nb]
    
    clf_nb = GaussianNB()
    clf_nb.fit(train_nb_x, train_nb_y)
    
    # DecisionTree
    train_df = zero[int(len(zero) * x1) : int(len(zero) * x2)] + one[int(len(one) * x1) : int(len(one) * x2)] + two[int(len(two) * x1) : int(len(two) * x2)]
    train_df_x = [row[:-1] for row in train_df]
    train_df_y = [row[-1] for row in train_df]
    
    clf_df = DecisionTreeClassifier()
    clf_df.fit(train_df_x, train_df_y)
    
    # RandomForest
    train_rf = zero[:int(len(zero) * x2)] + one[:int(len(one) * x2)] + two[:int(len(two) * x2)]
    train_rf_x = [row[:-1] for row in train_rf]
    train_rf_y = [row[-1] for row in train_rf]
    
    clf_rf = RandomForestClassifier(n_estimators=3)
    clf_rf.fit(train_rf_x, train_rf_y)
    ...
    for i in range(len(test_set)):
        ...
        pred_nb = clf_nb.predict([test_x[i]])
        pred_df = clf_df.predict([test_x[i]])
        pred_rf = clf_rf.predict([test_x[i]])
        
        if pred_nb[0] == test_y[i]:
            count += 1
        if pred_df[0] == test_y[i]:
            count += 1
        if pred_rf[0] == test_y[i]:
            count += 1
