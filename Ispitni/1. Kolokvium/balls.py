...
class Topki(Problem):
    ...
    def __init__(self, obstacles, N, initial, goal=None):
    ...
    def successor(self, state):
        ...
        for t in state:
            ...
            if x - 2 >= 0 and y + 2 < self.N:
                tmp_balls = list(state)
                if (x - 1, y + 1) in state and (x - 2, y + 2) not in state and (x - 2, y + 2) not in self.obstacles:
                    tmp_balls.remove(t)
                    tmp_balls.remove((x - 1, y + 1))
                    tmp_balls.append((x - 2, y + 2))
                    successors['Gore Levo: (x=' + str(x) + ',y=' + str(y) + ')'] = tuple(tmp_balls)
            ...
            if x + 2 < self.N and y + 2 < self.N:
                ...
                if (x + 1, y + 1) in state and (x + 2, y + 2) not in state and (x + 2, y + 2) not in self.obstacles:
                    ...
                    tmp_balls.remove((x + 1, y + 1))
                    tmp_balls.append((x + 2, y + 2))
                    successors['Gore Desno: (x=' + str(x) + ',y=' + str(y) + ')'] = tuple(tmp_balls)
            ...
            if x - 2 >= 0 and y - 2 >= 0:
                ...
                if (x - 1, y - 1) in state and (x - 2, y - 2) not in state and (x - 2, y - 2) not in self.obstacles:
                    ...
                    tmp_balls.remove((x - 1, y - 1))
                    tmp_balls.append((x - 2, y - 2))
                    successors['Dolu Levo: (x=' + str(x) + ',y=' + str(y) + ')'] = tuple(tmp_balls)
            ...
            if x + 2 < self.N and y - 2 >= 0:
                ...
                if (x + 1, y - 1) in state and (x + 2, y - 2) not in state and (x + 2, y - 2) not in self.obstacles:
                    ...
                    tmp_balls.remove((x + 1, y - 1))
                    tmp_balls.append((x + 2, y - 2))
                    successors['Dolu Desno: (x=' + str(x) + ',y=' + str(y) + ')'] = tuple(tmp_balls)


            # Right
            if x + 2 < self.N:
                tmp_balls = list(state)
                if (x + 1, y) in state and (x + 2, y) not in state and (x + 2, y) not in obstacles:
                    tmp_balls.remove(t)
                    tmp_balls.remove((x + 1, y))
                    tmp_balls.append((x + 2, y))
                    successors['Desno: (x=' + str(x) + ',y=' + str(y) + ')'] = tuple(tmp_balls)
            ...
            if x - 2 >= 0:
                ...
                if (x - 1, y) in state and (x - 2, y) not in state and (x - 2, y) not in obstacles:
                    ...
                    tmp_balls.remove((x - 1, y))
                    tmp_balls.append((x - 2, y))
                    successors['Levo: (x=' + str(x) + ',y=' + str(y) + ')'] = tuple(tmp_balls)
    ...
    def goal_test(self, state):
        if len(state) == 1:
            if state[0][0] == self.N // 2 and state[0][1] == self.N - 1:
                return True
        return False
...
if __name__ == "__main__":
    ...
    for i in range(num_balls):
        balls.append(tuple([int(j) for j in input().split(",")]))

    num_obstacles = int(input())

    for i in range(num_obstacles):
        obstacles.append(tuple([int(j) for j in input().split(",")]))

    topki = Topki(tuple(obstacles), dimenzii, tuple(balls))
    res = breadth_first_graph_search(topki)
    print(res.solution())