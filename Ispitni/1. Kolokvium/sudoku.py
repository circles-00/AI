...
class Squares(Problem):
    ...
    def h(self, node):
        ...
        one_x = one[0]
        one_y = one[1]
        value += manhattan(one_x, one_y, 0, 4)
        
        two = state[1]
        two_x = two[0]
        two_y = two[1]
        value += manhattan(two_x, two_y, 1, 3)
        
        tri = state[2]
        tri_x = tri[0]
        tri_y = tri[1]
        value += manhattan(tri_x, tri_y, 2, 2)
        
        four = state[3]
        four_x = four[0]
        four_y = four[1]
        value += manhattan(four_x, four_y, 3, 1)
        
        five = state[4]
        five_x = five[0]
        five_y = five[1]
        value += manhattan(five_x, five_y, 4, 0)
