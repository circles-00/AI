...
if __name__ == '__main__':
    ...
    tmp = [row[:-1] for row in train_set]
    train_x = []
    for row in tmp:
        pom = [row[i] for i in range(len(row)) if i != col_index]
        train_x.append(pom)
    train_y = [row[-1] for row in train_set]
    
    tmp = [row[:-1] for row in test_set]
    test_x = []
    for row in tmp:
        pom = [row[i] for i in range(len(row)) if i != col_index]
        test_x.append(pom)
    ...
    clf = RandomForestClassifier(n_estimators=est_read, criterion=crit_read)
